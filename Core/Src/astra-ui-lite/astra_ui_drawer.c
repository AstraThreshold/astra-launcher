...
void astra_draw_info_bar()
{
  ...
  oled_draw_R_box(OLED_WIDTH/2 - (astra_info_bar.w_info_bar + 4)/2, astra_info_bar.y_info_bar - 2, astra_info_bar.w_info_bar + 4, INFO_BAR_HEIGHT + 4, 3);
  ...
  oled_draw_R_frame(OLED_WIDTH/2 - astra_info_bar.w_info_bar/2, astra_info_bar.y_info_bar - 4, astra_info_bar.w_info_bar, INFO_BAR_HEIGHT + 4, 3);
...
void astra_draw_pop_up()
{
  ...
  if (astra_pop_up.time - astra_pop_up.time_start >= astra_pop_up.span && astra_pop_up.is_running)
  {
  ...
  oled_set_font(u8g2_font_wqy12_t_chinese1);
  oled_set_draw_color(0); //黑遮罩打底
  oled_draw_R_box(OLED_WIDTH/2 - (astra_pop_up.w_pop_up + 4)/2, astra_pop_up.y_pop_up - 2, astra_pop_up.w_pop_up + 4, POP_UP_HEIGHT + 4, 3);

  oled_set_draw_color(1);
  oled_draw_R_frame(OLED_WIDTH/2 - astra_pop_up.w_pop_up/2, astra_pop_up.y_pop_up, astra_pop_up.w_pop_up, POP_UP_HEIGHT, 3);

  oled_draw_UTF8(OLED_WIDTH/2 - astra_pop_up.w_pop_up/2 + 4, astra_pop_up.y_pop_up + oled_get_str_height() - 1, astra_pop_up.content);
}
